...
import InstalledDetail from './components/InstalledDetail';
...
export default class Installed extends React.Component{
    ...
    getTableList() {
        ...
        return new Promise((resolve, reject) => {
            request('/installed/list').then(response => {
    ...
    deleteRecord(record, e) {
        ...
        confirm({
            ...
            icon: null,
            content: <p className='_confirm-dialog-content confirm-warn-msg'>如果不确定是否要从系统中删除该应用，建议先冻结该应用</p>,
            onOk() {
    ...
    confirmDelete(id) {
        return new Promise((resolve, reject) => {
            ...
            request(`/installed/delete/${id}`, {method: 'delete'}).then(respond => {
                if(respond && respond.status === 0) {
                    message.success(`删除应用成功`);
                    return resolve();
                }
                message.warn(`删除应用失败，`, respond && respond.message || '响应异常');
                return reject();
            }).catch(e => {
                message.error(`删除应用失败，`, e.message || '网络异常');
                reject(e.message);
            });
        });
    }
